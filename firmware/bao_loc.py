...
def lcd_lines(snapshot, link=""):
    ...
    line1 = pad_row(
        "T:" + show("temperature", temperature, "{:.1f}C"),
        "H:" + show("humidity", humidity, "{:.1f}%")
    )

    line2 = pad_row(
        "PM2.5:" + show("pm2_5", pm2_5, "{}"),
        "PM10:" + show("pm10", pm10, "{}")
    )
    ...
    if wind_speed is None:
        ...
        line3 = pad_row("Wind:" + OUT_OF_RANGE, "")
        ...
        line3 = pad_row("W:{:.1f}m/s".format(wind_speed), OUT_OF_RANGE)
    else:
        line3 = pad_row(
    ...
    if light is None:
        ...
        lux_txt = "L" + OUT_OF_RANGE
    else:
        lux_txt = "L" + fmt_lux(light)

    noise_txt = "N" + show("noise", noise, "{:.0f}")
    pressure_txt = "P" + show("pressure", pressure, "{:.1f}")

    # Ba truong nay dai toi da 5 + 4 + 6 = 15 ky tu cong hai dau cach = 17, nen
    # con du cho ky tu link o cot 20 ma khong cham luoi an toan cua pad_row.
    # Truoc day phep tinh nay chi dung voi gia tri HOP LY va khong co gi bat
    # buoc dieu do; gio PLAUSIBLE moi la thu thuc su chan tren ("L200k" 5,
    # "N125" 4, "P120.0" 6). Ngoai dai thi thanh "!!!", con ngan hon.
    line4 = pad_row(
