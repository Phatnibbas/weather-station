...
# Write API Key của riêng channel Sài Gòn.
#
...
try:
    from station_secrets import (
        THINGSPEAK_WRITE_API_KEY_SAIGON as THINGSPEAK_WRITE_API_KEY,
    )
except ImportError:
    try:
        ...
        print("Them THINGSPEAK_WRITE_API_KEY_SAIGON = \"...\" vao file do.")
...
# Ky tu cuoi dong 2 = trang thai duong len cloud, NHIN TU TAI TRAM.
...
def lcd_lines(temperature, pm2_5, light, wind_speed, wind_angle, link=""):
    """
    ...
    Dong 2: nhiet do + PM2.5 + anh sang + 1 ky tu trang thai cloud
    Gia tri doc hong hien "--" chu khong giu lai so cu.

    `link` la trang thai cua lan upload TRUOC. Do la su that duy nhat co that o
    thoi diem ve man hinh, va noi dung nay chay truoc buoc upload trong moi chu
    ky. Khong duoc doan truoc ket qua lan gui sap toi.

    16 cot chat hon 20 nhieu, nen ky tu link duoc tinh vao ngan sach cua thang
    rut gon ben duoi chu KHONG duoc dan them vao roi de pad_row cat bot.
    """
    ...
    # rồi rút nhãn PM, cuối cùng mới bỏ tiền tố L của lux. Không bao giờ cắt
    # chuỗi - mất một chữ số thập phân hay một chữ cái nhãn thì nhìn ra ngay,
    # còn một con số bị cắt cụt vẫn đọc được như số thật và sẽ đánh lừa người
    # xem (200000 lux hiện thành "200").
    #
...
while True:
    ...
    if len(failed_reads) >= attempted_reads:
        lcd_write("SENSOR NO REPLY", pad_row("Check RS485/pwr", link))
    else:
    ...
    if should_restart(ticks_diff(ticks_ms(), boot_ticks)):
        ...
        lcd_write(
            "Scheduled restart",
            local_clock_text() if _clock_ok else "no NTP - uptime"
        )
